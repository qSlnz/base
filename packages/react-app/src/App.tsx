...
if (
  appNetwork.chainId !== deployedNetwork.chainId ||
  appNetwork.chainName !== deployedNetwork.name
) {
...
function App() {
  ...
  // UI datas
  const [activeLoadingScreen, setActiveLoadingScreen] = useState<boolean>(
    false
  );
  ...
  useEffect(() => {
    const getPools = async () => {
      ...
      let poolCount = (await contracts.StakerReader.getPoolCount()).toString();
      ...
      let creationPoolEventFilter = contracts.StakerReader.filters.PoolCreation(
        null,
        null
      );

      contracts.StakerReader.on(
        creationPoolEventFilter,
        async (poolId, address, event) => {
          console.log(
            "Event: creationPoolEventFilter: " +
              address +
              ", " +
              poolId +
              ", " +
          ...
          let newPoolBox = (
            <PoolBox poolId={cleanPoolId} address={address} key={cleanPoolId} />
          );
      ...
      <div className="mainscreen">
        ...
        <div className="loginbar">
          {onboard ? (
            <div className="loginbar-login">
              {!wallet.provider && (
                <div
                  className="loginbar-button connexion"
                  onClick={async () => {
                ...
                >
                  <div className="loginbar-button-text">Connexion</div>
                </div>
              )}

              {wallet.provider && !isNetworkCorrect && (
                <div
                  className="loginbar-button badnetwork"
                  onClick={async () => {
                    networkSwitcher();
                    await onboard.walletCheck();
                  }}
              ...
              {wallet.provider && isNetworkCorrect && (
                <a
                  className="loginbar-button-network"
                  style={{ backgroundColor: appNetwork.color }}
                  target="_blank"
                  href={appNetwork.faucet}
                  rel="noreferrer"
                >
              ...
              {wallet.provider && isNetworkCorrect && (
                <a
                  className="loginbar-button connected"
                  target="_blank"
                  href={appNetwork.blockExplorer + "address/" + address}
                  rel="noreferrer"
                >
                  ...
                  <div className="loginbar-button-connect-wrapper">
                    <span className="loginbar-button-address">
                      {ens.substr(0, 1).toUpperCase() + ens.substr(1, 11) ||
                        address.substr(0, 6) +
                          "..." +
              ...
              {wallet.provider && (
                <div
                  className="loginbar-disconnect-button"
                  onClick={async () => {
        ...
        {wallet.provider && network && isNetworkCorrect ? (
          ...
          <div className="appZone" style={{ backgroundColor: "royalblue" }}>
            <div className="appZone-text">Pooling made easy.</div>
          </div>
        )}
      </div>
    </div>
